# Hi, I'm Afif Alfiano 👋

**Senior Frontend Engineer** based in Yogyakarta, Indonesia 🇮🇩  
Specializing in **Angular · TypeScript · RxJS** — building scalable web & mobile apps.

Currently working remotely at **Jublia** (Singapore) — an AI-native event platform that powers engagement intelligence for tradeshows and conferences worldwide.

---

### What I work with

**Frontend** — Angular, TypeScript, RxJS, Ionic/Capacitor, React, Vue  
**Backend** — Node.js, Express, PostgreSQL, Prisma  
**Infra** — Docker, GitHub Actions, Nginx, VPS (Ubuntu)  
**Observability** — Grafana, Grafana Faro  

---

### Things I've built

- **[Caca](https://afifalfiano.medium.com)** — Telegram AI finance assistant built with Claude Haiku, Google Sheets API & an agentic runtime
- **[Mood Tracker](https://mood-tracker.pipdev.net)** — Full-stack personal app with Vite + Express + PostgreSQL + Docker, deployed via CI/CD
- **[filter-ticket](https://npmjs.com/package/filter-ticket)** — Angular library for dynamic ticket filtering
- **[monitor-subscription](https://npmjs.com/package/monitor-subscription)** — RxJS subscription leak detector for Angular
- **TamaPip** — Tamagotchi-style browser pet game (React, fully spec'd with PRD + TRD)

---

### Writing

I write about frontend engineering, AI tooling, and developer workflows.  
→ [afifalfiano.medium.com](https://afifalfiano.medium.com)

---

### Let's connect

[Portfolio](https://pipdev.net) · [LinkedIn](https://linkedin.com/in/afifalfiano) · [Email](mailto:afifalfiano2@gmail.com)

---

*Open to Senior Frontend roles in Switzerland 🇨🇭, Germany 🇩🇪, or remote worldwide. Visa sponsorship needed.*
